...
const App: React.FC = () => {
  ...
  // Fix: Establish real communication with the Figma plugin backend
  useEffect(() => {
  ...
  // Fix: Fetch variables when source collection is selected
  const handleSourceSelect = (id: string) => {
    setState(prev => ({ ...prev, sourceCollectionId: id, step: 'VARIABLES' }));
    parent.postMessage({ pluginMessage: { type: 'GET_VARIABLES', payload: { collectionId: id } } }, '*');
  };

  // Fix: Generate AI analysis on success using Gemini
  useEffect(() => {
  ...
  const handleSelectAll = () => {
    setState(prev => ({ ...prev, selectedVariableIds: variables.map(v => v.id) }));
  };
  ...
  const runMigration = () => {
    ...
    // Fix: Send real migration command to code.ts
    parent.postMessage({ 
      pluginMessage: { 
        type: 'RUN_MIGRATION', 
        payload: {
  ...
  const sourceCollection = collections.find(c => c.id === state.sourceCollectionId);
    ...
    <div className="flex flex-col h-screen bg-[#0C0C0C] text-white">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-[#2C2C2C] bg-[#121212] shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold tracking-tight">FigSystem</h1>
        </div>
        <div className="flex items-center gap-3">
          ...
          <RotateCw size={18} className="text-gray-400 cursor-pointer hover:text-white transition-colors" onClick={() => parent.postMessage({ pluginMessage: { type: 'GET_DATA' } }, '*')} />
          <MoreVertical size={18} className="text-gray-400 cursor-pointer hover:text-white" />
        </div>
      </header>
          ...
          <div className="flex h-full">
            ...
            <aside className="w-64 border-r border-[#2C2C2C] bg-[#121212] overflow-y-auto">
              <div className="p-4 flex items-center justify-between border-b border-[#2C2C2C]">
                <div className="flex items-center gap-2">
                  <ArrowLeft size={16} className="cursor-pointer text-gray-400" onClick={() => setState(s => ({ ...s, step: 'SOURCE' }))} />
                  <span className="font-semibold text-sm truncate max-w-[120px]">{sourceCollection?.name}</span>
                </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden bg-[#0C0C0C]">
              <div className="p-4 border-b border-[#2C2C2C] flex items-center justify-between">
                <h3 className="text-sm font-medium text-gray-300">Select the variables you want to move</h3>
              </div>
                  ...
                  <thead className="sticky top-0 bg-[#121212] text-gray-500 uppercase text-[10px] tracking-wider z-10">
                    <tr>
                      <th className="px-6 py-2 border-b border-[#2C2C2C] w-12">
                        <input type="checkbox" className="rounded bg-[#2C2C2C] border-none" checked={state.selectedVariableIds.length === variables.length && variables.length > 0} onChange={state.selectedVariableIds.length === variables.length ? handleDeselectAll : handleSelectAll} />
                      </th>
                      ...
                      >
                        <td className="px-6 py-4 border-b border-[#1A1A1A]">
                          <div className={`w-5 h-5 rounded border ${state.selectedVariableIds.includes(v.id) ? 'bg-[#7B61FF] border-[#7B61FF]' : 'border-[#444]'} flex items-center justify-center`}>
                            {state.selectedVariableIds.includes(v.id) && <Check size={14} className="text-white" />}
                          </div>
                        </td>
              ...
              <footer className="p-4 bg-[#121212] border-t border-[#2C2C2C] flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4 text-xs text-gray-400">
                  ...
                  <button onClick={handleSelectAll} className="hover:text-white transition-colors">Select All</button>
                  <button onClick={handleDeselectAll} className="hover:text-white transition-colors">Deselect All</button>
                </div>
                <button 
        ...
        {state.step === 'TARGET' && (
          <div className="p-6 max-w-3xl mx-auto flex flex-col h-full">
             <div className="flex justify-between items-center mb-6">
               ...
               <button onClick={() => setState(s => ({ ...s, step: 'VARIABLES' }))} className="p-2 rounded-full hover:bg-[#1E1E1E]">
                  <ArrowLeft size={20} />
               </button>
             </div>

             <div className="flex-1 overflow-y-auto space-y-2 mb-6">
                {collections.filter(c => c.id !== state.sourceCollectionId).map(col => (
                  <button
                    key={col.id}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <span className="text-gray-500 text-sm">{col.variableIds.length}</span>
                      {state.targetCollectionId === col.id && <Check size={18} className="text-[#7B61FF]" />}
                    </div>
                  </button>
                ))}
             </div>
          ...
          <div className="flex flex-col items-center justify-center h-full p-6 text-center">
            {state.step === 'MIGRATING' ? (
              <>
                ...
                <p className="text-gray-400 max-w-sm">Performing two-pass migration to preserve data integrity and rebinding nodes.</p>
              ...
              <>
                ...
                <p className="text-gray-400 max-w-sm mb-6">All variables have been moved and references updated across your design.</p>
                ...
                <button 
                  onClick={() => setState({ sourceCollectionId: null, selectedVariableIds: [], targetCollectionId: null, step: 'SOURCE' })}
